...
export default function Contact() {
    ...
    useEffect(() => {
        setLoading(true)
        axios.get(`${import.meta.env.VITE_HOST}/admin/fetch-contacts?page=${page}`)
            .then(res => {
                const { status, data } = res
                if (status === 200) {
                    setReports(data.contacts)
                    setTotalPages(Math.ceil(data.totalContacts / 20))
                }
            })
            .catch(err => {
    ...
    const handleUpdateFn = async () => {
        ...
        try {
            ...
            const res = await axios.patch(`${import.meta.env.VITE_HOST}/admin/contact/update/${clickedReport._id}`, { status });
    ...
    const handleDelete = async id => {
        ...
        setLoading(true)
        try {
            const res = await axios.delete(`${import.meta.env.VITE_HOST}/admin/contact/delete/${id}`)
            if (res.status === 203) {
    ...
    return (
        <div className='main-container'>
            <h3 className='flex gap-2 items-center'><FaAddressCard /> Contact Messages</h3>
            <p className='px-2 py-1 bg-[var(--md-light)] inline-block rounded-[8px] !text-[var(--dark)] mt-2'>Manage contact messages of your users</p>
                        ...
                        {reports.length > 0 ? (
                            reports.map((report, i) => (
                                <tr key={report._id} className='hover:bg-gray-50'>
                                    ...
                                    <td className='p-4'>{report.topic}</td>
